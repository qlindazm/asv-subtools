...
class Xvector(TopVirtualNnet):
    ...
    def init(self, inputs_dim, num_targets, extend=False, skip_connection=False, 
        ...
        self.tdnn6_1 = ReluBatchNormTdnnLayer(stats_dim, 512, **tdnn_layer_params)
        self.tdnn6_2 = ReluBatchNormTdnnLayer(stats_dim, 512, **tdnn_layer_params)
        ...
        self.tdnn7_1 = ReluBatchNormTdnnLayer(tdnn7_dim,512, **tdnn7_params)
        self.tdnn7_2 = ReluBatchNormTdnnLayer(tdnn7_dim,512, **tdnn7_params)
        ...
        if training :
            if margin_loss:
                ...
                self.loss = BiAamSoftmaxLoss(512, num_targets, **margin_loss_params)
            else:
    ...
    def forward(self, inputs):
        ...
        x1 = self.auto(self.tdnn6_1, x)
        x1 = self.tdnn7_1(x1)
        x2 = self.auto(self.tdnn6_2, x)
        x2 = self.tdnn7_2(x2)
    ...
    def get_orth_loss(self, inputs, targets):
        # inputs&targets are not used, just for calculate gradient
    ...
    def extract_embedding(self, inputs):
        ...
        if self.extracted_embedding == "far" :
            ...
            # xvector = (self.tdnn6_1.affine(x) + self.tdnn6_2.affine(x) ) * 0.5
            xvector = torch.cat((self.tdnn6_1.affine(x), self.tdnn6_2.affine(x)), dim=1) # better
...
if __name__ == '__main__':
    ...
    model = Xvector(23, 10, margin_loss=True)
    inputs = torch.randn(8, 23, 100)
    tagets = torch.tensor([i for i in range(8)])
    inputs = model(inputs)
    # print(inputs.shape)
    print(model.get_loss(inputs, tagets))