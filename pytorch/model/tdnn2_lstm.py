...
class Xvector(TopVirtualNnet):
    ...
    def init(self, inputs_dim, num_targets, extend=False, skip_connection=False, 
        ...
        self.tdnn21 = ReluBatchNormTdnnLayer(inputs_dim,512,[-2,-1,0,1,2],nonlinearity=tdnn_layer_params["nonlinearity"])
        self.tdnn22 = ReluBatchNormTdnnLayer(512,512,[-2,0,2],nonlinearity=tdnn_layer_params["nonlinearity"])
        self.tdnn23 = ReluBatchNormTdnnLayer(512,512,[-3,0,3],nonlinearity=tdnn_layer_params["nonlinearity"])
        self.lstm = nn.LSTM(input_size=512, hidden_size=512, num_layers=1)
        self.tdnn24 = ReluBatchNormTdnnLayer(512,512,nonlinearity=tdnn_layer_params["nonlinearity"])
        self.tdnn25 = ReluBatchNormTdnnLayer(512,1500,nonlinearity=tdnn_layer_params["nonlinearity"])

        self.linear = nn.Linear(3000, 1500)
    ...
    def forward(self, inputs):
        ...
        #tdnn_lstm
        x2 = inputs
        x2 = self.auto(self.aug_dropout, x2)
        x2 = self.tdnn21(x2)   
        x2 = self.tdnn22(x2)
        x2 = self.tdnn23(x2)
        x2, _ = self.lstm(x2.permute(2, 0, 1))
        x2 = self.tdnn24(x2.permute(1, 2, 0))
        x2 = self.tdnn25(x2)

        x = torch.cat([x, x2], 1) # shape (batch, 3000, frame)
        x = self.linear(x.permute(0, 2, 1))

        x = self.stats(x.permute(0, 2, 1))
