                            ...
                            try:
                                ...
                                for i in range(weight.shape[0]):
                                    for j in range(i+1, weight.shape[0]):
                                        orth_snapshot["orth_snp"] += torch.dot(weight[i], weight[j]).item()
                                orth_snapshot["orth_snp"] /= weight.shape[0] * (weight.shape[0]-1) / 2
...
class TwoStepTrainer(_BaseTrainer):
    """One input and one output.
    """
    def __init__(self, *args, **kwargs):
        super(TwoStepTrainer, self).__init__(*args, **kwargs)
        self.elements["optimizer2"] = args[0][0]["optimizer2"]
    ...
    def train_one_batch(self, batch):
        ...
        model_forward = self.elements["model_forward"]
        optimizer = self.elements["optimizer"]
        optimizer2 = self.elements["optimizer2"]
        ...
        orth_loss = model.get_orth_loss(model_forward(inputs), targets)
        orth_loss.backward()
        orth_loss.detach() # For safe.
        optimizer2.step()
    ...
    def lr_finder_compute(self, train_batch):
        ...
        return ["train_loss", "train_acc", "valid_loss", "valid_acc", "orth"], [loss, acc, valid_loss, valid_acc, orth]
